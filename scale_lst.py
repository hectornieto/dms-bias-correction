...
import agrotig_input_creator.gdal_utils as gu
import agrotig_input_creator.fmask_utils as fu
import numpy as np
...
def plot_results(ref, orig, corr, outfile, q=20):
    """
...
def rescale_lst(landsat_folder, target_file, ref_date, tiles,
    ...
    valid = np.logical_and(np.isfinite(lst_dms), np.isfinte(scale))
...
def composite_reference_variability(reference_folder, ref_date, image_template, tiles,
    ...
    max_days_offset : int
        ...
        If positive only search backwards in the past of ref_date, for NRT applications
        If negative search both backwards and forwards in the future of ref_date, for NTC applications
    tiles : list
...
def local_variability(input_image, image_template, window_size=(1000, 1000)):
    ...
    valid = fu.reclassify_espamask(raster, buffer_clouds=1, buffer_snow=0,
                                   buffer_shadow=0, buffer_water=0)
...
def downsample_dataset(ref_array, orig_array, corr_array,
                       n_clusters=5, subsample=1):

    n_cases = ref_array.shape
    sampling = np.random.uniform(size=np.sum(n_cases))
    sampling = sampling <= subsample
    ref_array = ref_array[sampling]
    orig_array = orig_array[sampling]
    corr_array = corr_array[sampling]
    clusters = KMeans(n_clusters=n_clusters).fit(np.concatenate([ref_array.reshape(-1, 1),
                                                                 orig_array.reshape(-1, 1),
                                                                 corr_array.reshape(-1, 1)], axis=1)
                                                 )
    ...
    for i in range(n_clusters):
        ...
        if i == 0:
            sampled_ref = ref_array[sampled]
            sampled_orig = orig_array[sampled]
            sampled_corr = corr_array[sampled]
        else:
            sampled_ref = np.append(sampled_ref, ref_array[sampled], axis=0)
            sampled_orig = np.append(sampled_orig, orig_array[sampled], axis=0)
            sampled_corr = np.append(sampled_corr, corr_array[sampled], axis=0)
    print(f"Downsampled values to {np.size(sampled_ref)} cases")

    return sampled_ref, sampled_orig, sampled_corr
...
if __name__ == "__main__":
    landsat_dir = Path("/mnt/lvstorage/landsat/")
    tiles = ["199033", "200033"]
    workdir = Path() / "test"
    dms_dir = workdir / "dms_raw"
    out_dir = workdir / "dms_corr_best_case"
    final_correction = True
    out_dir_final = workdir / "dms_corr_best_case_final"
    ...
    for dms_file in dms_files:
        ...
        if subset > 0:
            lst_ref_subset, lst_orig_subset, lst_corr_subset = downsample_dataset(lst_ref,
                                                             lst_orig,
                                                             lst_corr,
                                                             subsample=subset)

            orig_all += lst_orig_subset.tolist()
            ref_all += lst_ref_subset.tolist()
            corr_all += lst_corr_subset.tolist()
            del lst_ref_subset, lst_orig_subset, lst_corr_subset
        else:
            orig_all += lst_orig.tolist()
            ref_all += lst_ref.tolist()
            corr_all += lst_corr.tolist()

        outfile = out_dir_final / f"DMS_comparison_{datestr}.png"
        print(f"Plotting comparison graphs for {datestr}")
        plot_results(lst_ref, lst_orig, lst_corr, outfile, q=10)

        del lst_ref, lst_orig, lst_corr

    datestr = "All dates"
    print(f"Plotting comparison graphs for {datestr}")
    outfile = out_dir_final / f"DMS_comparison_all.png"
    plot_results(np.asarray(ref_all), np.asarray(orig_all), np.asarray(corr_all),
                 outfile, q=5)
